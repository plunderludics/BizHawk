...
internal class EmulatorInstance {
    ...
    public bool InitEmulator(
        ...
        // Init controls
        inputManager.ControllerInputCoalescer = new ControllerInputCoalescer(); // [seems to be necessary]
        ...
        if (loaded) {
            ...
            if (!String.IsNullOrEmpty(saveStateFileName)) {
                ...
                if (!loadedState) {
                    Console.WriteLine($"Warning: Failed to load state: {saveStateFileName}");
                }
            }

            // luaEngine.Restart(config, inputManager, emulator, game, luaScriptPaths);
        } else {
            Console.WriteLine($"Warning: Failed to load rom: {romPath}.");
        }
        return loaded;
    }
    ...
    public bool LoadState(string path) {
        ...
        {
            Console.WriteLine($"Warning: Could not load state: {path}");
            return false;
        }
    ...
    CoreComm CreateCoreComm() {
        ...
        return new CoreComm(
            s => Console.WriteLine($"message: {s}"),
            s => Console.WriteLine($"notification: {s}"),
            cfp,
            prefs);
    }
