...
namespace BizHawk.Client.Common
{
	...
	public static class ArgParser
	{
		...
		// [UnityHawk] UnityHawk-specific options
		private static readonly BespokeOption<string?> OptionCustomWindowTitle = new(name: "--windowtitle", description: "[UnityHawk] custom window title");
		private static readonly BespokeOption<string?> OptionFirmwareDir = new(name: "--firmware", description: "[UnityHawk] firmware directory path");
		private static readonly BespokeOption<string?> OptionExtToolsDir = new(name: "--ext-tools-dir", description: "[UnityHawk] external tools directory path");
		private static readonly BespokeOption<string?> OptionSavestateSaveDir = new(name: "--savestates", description: "[UnityHawk] savestate save directory path");
		private static readonly BespokeOption<string?> OptionSavestateExtension = new(name: "--savestate-extension", description: "[UnityHawk] savestate file extension");
		private static readonly BespokeOption<string?> OptionRamWatchFile = new(name: "--ram-watch-file", description: "[UnityHawk] RAM watch file path");
		private static readonly BespokeOption<string?> OptionRamWatchSaveDir = new(name: "--save-ram-watch", description: "[UnityHawk] RAM watch save directory path");
		private static readonly BespokeOption<bool> OptionHeadless = new(name: "--headless", description: "[UnityHawk] don't open any GUI at all");
		private static readonly BespokeOption<string?> OptionAcceptBackgroundInput = new(name: "--accept-background-input", description: "[UnityHawk] accept input when window is in background; value should be 'true' or 'false'");
		private static readonly BespokeOption<bool> OptionSuppressPopups = new(name: "--suppress-popups", description: "[UnityHawk] suppress popup dialogs");
		private static readonly BespokeOption<string?> OptionMute = new(name: "--mute", description: "[UnityHawk] mute audio; value should be 'true' or 'false'");
		...
		static ArgParser()
		{
			...
			// [UnityHawk] UnityHawk-specific options
			root.Add(/* --windowtitle */ OptionCustomWindowTitle);
			root.Add(/* --firmware */ OptionFirmwareDir);
			root.Add(/* --ext-tools-dir */ OptionExtToolsDir);
			root.Add(/* --savestates */ OptionSavestateSaveDir);
			root.Add(/* --savestate-extension */ OptionSavestateExtension);
			root.Add(/* --ram-watch-file */ OptionRamWatchFile);
			root.Add(/* --save-ram-watch */ OptionRamWatchSaveDir);
			root.Add(/* --headless */ OptionHeadless);
			root.Add(/* --accept-background-input */ OptionAcceptBackgroundInput);
			root.Add(/* --suppress-popups */ OptionSuppressPopups);
			root.Add(/* --mute */ OptionMute);
...
//				.CancelOnProcessTermination() // we're only using the parser, so there's not really anything to cancel
		...
		public static int? ParseArguments(out ParsedCLIFlags parsed, string[] args)
		{
			...
			// [UnityHawk] Parse UnityHawk-specific options
			var customWindowTitle = result.GetValueForOption(OptionCustomWindowTitle);
			var firmwareDir = result.GetValueForOption(OptionFirmwareDir);
			var extToolsDir = result.GetValueForOption(OptionExtToolsDir);
			var savestateSaveDir = result.GetValueForOption(OptionSavestateSaveDir);
			var savestateExtension = result.GetValueForOption(OptionSavestateExtension);
			var ramWatchFile = result.GetValueForOption(OptionRamWatchFile);
			var ramWatchSaveDir = result.GetValueForOption(OptionRamWatchSaveDir);
			var headless = result.GetValueForOption(OptionHeadless);
			// For --accept-background-input and --mute, parse string values as bool
			bool? acceptBackgroundInput = null;
			if (result.GetValueForOption(OptionAcceptBackgroundInput) is string abiStr)
			{
				acceptBackgroundInput = abiStr.EqualsIgnoreCase("true");
			}
			var suppressPopups = result.GetValueForOption(OptionSuppressPopups);
			bool? mute = null;
			if (result.GetValueForOption(OptionMute) is string muteStr)
			{
				mute = muteStr.EqualsIgnoreCase("true");
			}
